	...
	fn default() -> ExtBuilder {
		ExtBuilder {
			...
			inflation: InflationInfoWithoutRound {
				...
				// not used
				annual: Range { min: Perbill::from_percent(50), ideal: Perbill::from_percent(50) },
			},
		}
	}
}
...
fn geneses() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// collators
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&1), Ok(500));
			...
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&2), Ok(100));
			...
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&7), Ok(100));
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&8), Ok(9));
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&9), Ok(4));
		});
	ExtBuilder::default()
		.with_balances(vec![
			(1, 100),
		...
		.execute_with(|| {
			...
			for x in 1..5 {
				...
				assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&x), Ok(80));
			}
			assert!(ParachainStaking::is_candidate(&5));
			assert_eq!(query_lock_amount(5, COLLATOR_LOCK_ID), Some(10));
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&5), Ok(90));
			// delegators
			for x in 6..11 {
				assert!(ParachainStaking::is_delegator(&x));
...
fn test_assert_events_emitted_match_passes_if_all_events_present_multiple() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_events_emitted_match!(
			ParachainStakingEvent::CollatorChosen {
				total_exposed_amount: Stake { amount: 10, power: 10 },
				..
			},
			ParachainStakingEvent::Rewarded { account: 1, rewards: 100 },
		);
