...
use ink::{env::call::Selector, prelude::collections::BTreeMap};
...
mod mmr {
	...
	impl<T, M> MMR<T, M> {
		pub fn new(mmr_size: u64) -> Self {
			MMR { mmr_size, batch: Vec::new(), merge: PhantomData }
		}
	...
	impl<T: Clone + PartialEq + Encode + Packed + EncodeLike, M: Merge<Item = T>> MMR<T, M> {
		...
		pub fn get_root(&self, store: &Map<T>) -> Result<T> {
			...
			let peaks: Vec<T> = get_peaks(self.mmr_size)
				.into_iter()
				.map(|peak_pos| {
					store.get(&peak_pos).map(|v| v.clone()).ok_or(Error::InconsistentStore)
				})
		...
		fn gen_proof_for_peak(
			...
			if pos_list.is_empty() {
				proof
					.push(store.get(&peak_pos).map(|v| v.clone()).ok_or(Error::InconsistentStore)?);
				return Ok(())
			}
			...
			while let Some((pos, height)) = queue.pop_front() {
				...
				if Some(&sib_pos) == queue.front().map(|(pos, _)| pos) {
					...
					proof.push(
						store.get(&sib_pos).map(|v| v.clone()).ok_or(Error::InconsistentStore)?,
					);
				}
	...
	impl<T: Clone + PartialEq + Encode + Packed + EncodeLike, M: Merge<Item = T>> MerkleProof<T, M> {
		pub fn new(mmr_size: u64, proof: Vec<T>) -> Self {
			MerkleProof { mmr_size, proof, merge: PhantomData }
		}
	}
	...
	impl StateAggregator {
		...
		pub fn generate_proof(&self, positions: Vec<u64>) -> crate::GenerateProofReturn {
			...
			match proof {
				...
				Ok(proof) => Ok(MerkleProof { mmr_size: proof.mmr_size, proof: proof.proof }),
			}
		}
	...
	mod tests {
		...
		fn test_unauthorized_configure() {
			...
			assert!(state_aggregator
				.configure(vec![ConfigureArgument::SetAuthorizedProvider(data_provider,)])
				.is_err());
		}
