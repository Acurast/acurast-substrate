	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_emitted!(Event::NewRound {
				...
				total_balance: Stake { power: 20, amount: 20 }
			},);
			roll_to(17);
			assert_ok!(ParachainStaking::set_blocks_per_round(RuntimeOrigin::root(), 6u32));
			roll_to(18);
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			ParachainStaking::set_inflation(
				RuntimeOrigin::signed(45),
				Range { min: Perbill::from_percent(3), ideal: Perbill::from_percent(4) }
			),
			sp_runtime::DispatchError::BadOrigin
		);
		assert_noop!(
...
fn set_staking_event_emits_event_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		InflationConfig::<Test>::mutate(|cfg| {
			cfg.round = Range { min: Perbill::from_percent(5), ideal: Perbill::from_percent(5) }
		});
		// valid call succeeds
...
fn set_staking_updates_storage_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(ParachainStaking::inflation_config().ideal_staked, Perbill::from_percent(70));
		assert_ok!(ParachainStaking::set_staking_expectation(
			RuntimeOrigin::root(),
			Perbill::from_percent(60),
			None
		));
		assert_eq!(ParachainStaking::inflation_config().ideal_staked, Perbill::from_percent(60));
	});
}
...
fn set_inflation_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(ParachainStaking::set_inflation(RuntimeOrigin::root(), Range { min, ideal }));
...
fn set_inflation_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			...
			Range { min: Perbill::from_percent(50), ideal: Perbill::from_percent(50) }
		);
		assert_eq!(
			ParachainStaking::inflation_config().round,
			Range { min: Perbill::from_percent(5), ideal: Perbill::from_percent(5) }
		);
		assert_ok!(ParachainStaking::set_inflation(RuntimeOrigin::root(), Range { min, ideal }),);
		assert_eq!(ParachainStaking::inflation_config().annual, Range { min, ideal });
		assert_eq!(
			ParachainStaking::inflation_config().round,
			Range { min: Perbill::from_parts(57), ideal: Perbill::from_parts(75) }
		);
	});
}
...
fn cannot_set_invalid_inflation() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			ParachainStaking::set_inflation(
				RuntimeOrigin::root(),
				Range { min: Perbill::from_percent(5), ideal: Perbill::from_percent(4) }
			),
...
fn join_candidates_event_emits_correctly() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		...
		assert_events_eq!(Event::JoinedCollatorCandidates {
			account: 1,
			amount_locked: Stake { power: 10, amount: 10 },
			new_total: Stake { power: 10, amount: 10 },
		});
	});
}
...
fn join_candidates_reserves_balance() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&1), Ok(10));
		assert_ok!(ParachainStaking::join_candidates(RuntimeOrigin::signed(1), 10u128, 0u32));
		assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&1), Ok(0));
	});
}
...
fn join_candidates_increases_total_staked() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		assert_eq!(ParachainStaking::total(), Stake { power: 0, amount: 0 });
		assert_ok!(ParachainStaking::join_candidates(RuntimeOrigin::signed(1), 10u128, 0u32));
		assert_eq!(ParachainStaking::total(), Stake { power: 10, amount: 10 });
	});
}
...
fn join_candidates_creates_candidate_state() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		...
		assert_eq!(candidate_state.stake, Stake { power: 10, amount: 10 });
	});
}
...
fn join_candidates_adds_to_candidate_pool() {
	ExtBuilder::default().with_balances(vec![(1, 10)]).build().execute_with(|| {
		...
		assert_eq!(candidate_pool.0[0].stake, Stake { power: 10, amount: 10 });
	});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_emitted!(Event::CandidateLeft {
				ex_candidate: 1,
				unlocked_amount: Stake { power: 10, amount: 10 },
				new_total: Stake { power: 0, amount: 0 }
			});
		});
}
		...
		.execute_with(|| {
			...
			assert_eq!(
				...
				vec![ScheduledRequest {
					delegator: 2,
					when_executable: 3,
					action: DelegationAction::Decrease(Stake { power: 5, amount: 5 }),
				}],
			);
			assert_ok!(ParachainStaking::schedule_leave_candidates(RuntimeOrigin::signed(1), 1u32));
			// candidate state is not immediately removed
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_eq!(Event::CandidateBondedMore {
				candidate: 1,
				bond: Stake { power: 30, amount: 30 },
				new_total_bond: Stake { power: 50, amount: 50 }
			});
		});
}
...
fn candidate_bond_more_reserves_balance() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&1), Ok(30));
			assert_ok!(ParachainStaking::candidate_bond_more(RuntimeOrigin::signed(1), 30));
			assert_eq!(ParachainStaking::get_collator_stakable_free_balance(&1), Ok(0));
		});
}
...
fn candidate_bond_more_increases_total() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::candidate_bond_more(RuntimeOrigin::signed(1), 30));
			total += Stake { power: 30, amount: 30 };
...
fn candidate_bond_more_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(candidate_state.stake, Stake { power: 20, amount: 20 });
			assert_ok!(ParachainStaking::candidate_bond_more(RuntimeOrigin::signed(1), 30));
			let candidate_state = ParachainStaking::candidate_info(1).expect("updated => exists");
			assert_eq!(candidate_state.stake, Stake { power: 50, amount: 50 });
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_eq!(Event::CandidateBondedLess {
				candidate: 1,
				bond: Stake { power: 30, amount: 30 },
				new_bond: Stake { power: 20, amount: 20 }
			});
		});
}
...
fn execute_candidate_bond_less_decreases_total() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			roll_to(10);
			assert_ok!(ParachainStaking::execute_candidate_bond_less(RuntimeOrigin::signed(1), 1));
			total -= Stake { power: 10, amount: 10 };
			assert_eq!(ParachainStaking::total(), total);
		});
}
...
fn execute_candidate_bond_less_updates_candidate_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			let candidate_state = ParachainStaking::candidate_info(1).expect("updated => exists");
			assert_eq!(candidate_state.stake, Stake { power: 30, amount: 30 });
			assert_ok!(ParachainStaking::schedule_candidate_bond_less(
				RuntimeOrigin::signed(1),
				10
			));
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_eq!(Event::Delegation {
				delegator: 2,
				locked_amount: Stake { power: 10, amount: 10 },
				candidate: 1,
				delegator_position: DelegatorAdded::AddedToTop {
					new_total: Stake { power: 40, amount: 40 }
				},
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(
				...
				Ok(Stake { power: 10, amount: 10 })
			);
			assert_ok!(ParachainStaking::delegate(RuntimeOrigin::signed(2), 1, 10, 0, 0));
			assert_eq!(
				ParachainStaking::get_delegator_stakable_free_balance(&2),
				Ok(Stake { power: 0, amount: 0 })
			);
		});
}
...
fn delegate_updates_delegator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(delegator_state.total(), Stake { power: 10, amount: 10 });
			assert_eq!(delegator_state.delegations.0[0].owner, 1);
			assert_eq!(delegator_state.delegations.0[0].stake, Stake { power: 10, amount: 10 });
		});
}
...
fn delegate_updates_collator_state() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(candidate_state.total_stake_counted, Stake { power: 30, amount: 30 });
			let top_delegations =
			...
			assert_eq!(candidate_state.total_stake_counted, Stake { power: 40, amount: 40 });
			let top_delegations =
				ParachainStaking::top_delegations(1).expect("just delegated => exists");
			assert_eq!(top_delegations.delegations[0].owner, 2);
			assert_eq!(top_delegations.delegations[0].stake, Stake { power: 10, amount: 10 });
			assert_eq!(top_delegations.total, Stake { power: 10, amount: 10 });
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_emitted!(Event::DelegationKicked {
				delegator: 10,
				candidate: 1,
				unstaked_amount: Stake { power: 10, amount: 10 }
			});
...
fn execute_leave_delegators_decreases_total_staked() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_eq!(ParachainStaking::total(), Stake { power: 40, amount: 40 });
			assert_ok!(ParachainStaking::schedule_leave_delegators(RuntimeOrigin::signed(2)));
			roll_to(10);
			assert_ok!(ParachainStaking::execute_leave_delegators(RuntimeOrigin::signed(2), 2, 1));
			assert_eq!(ParachainStaking::total(), Stake { power: 30, amount: 30 });
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 2..6 {
				let candidate_state =
					ParachainStaking::candidate_info(i).expect("initialized in ext builder");
				assert_eq!(candidate_state.total_stake_counted, Stake { power: 20, amount: 20 });
				let top_delegations =
					ParachainStaking::top_delegations(i).expect("initialized in ext builder");
				assert!(top_delegations.delegations.is_empty());
			}
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				Event::DelegatorLeftCandidate {
					...
					candidate: 1,
					unstaked_amount: Stake { power: 10, amount: 10 },
					total_candidate_power: Stake { power: 30, amount: 30 }
				},
...
fn delegator_bond_more_increases_total_staked() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_eq!(ParachainStaking::total(), Stake { power: 40, amount: 40 });
			assert_ok!(ParachainStaking::delegator_bond_more(RuntimeOrigin::signed(2), 1, 5));
			assert_eq!(ParachainStaking::total(), Stake { power: 45, amount: 45 });
		});
}
...
fn delegator_bond_more_updates_candidate_state_bottom_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				ParachainStaking::bottom_delegations(1).expect("exists").delegations[0].stake,
				Stake { power: 10, amount: 10 }
			);
			assert_eq!(
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_eq!(Event::DelegationDecreaseScheduled {
				delegator: 2,
				candidate: 1,
				amount_to_decrease: Stake { power: 5, amount: 5 },
				execute_round: 3,
			});
		});
}
...
fn execute_delegator_bond_less_decreases_total_staked() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(ParachainStaking::total(), Stake { power: 35, amount: 35 });
		});
}
...
fn execute_delegator_bond_less_updates_just_bottom_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for Bond { owner, stake: amount } in pre_call_bottom_delegations.delegations {
				for Bond { owner: post_owner, stake: post_amount } in
			...
			assert!(not_equal);
			let mut equal = true;
			for Bond { owner, stake: amount } in pre_call_top_delegations.delegations {
				for Bond { owner: post_owner, stake: post_amount } in
...
fn execute_delegator_bond_less_does_not_delete_bottom_delegations() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(not_equal);
			assert_eq!(
				pre_call_candidate_info.total_stake_counted - Stake { power: 4, amount: 4 },
				post_call_candidate_info.total_stake_counted
			);
		});
}
		...
		.execute_with(|| {
			...
			assert_events_emitted!(Event::CancelledDelegationRequest {
				...
				cancelled_request: CancelledScheduledRequest {
					when_executable: 3,
					action: DelegationAction::Revoke(Stake { power: 10, amount: 10 }),
				},
			});
		});
}
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				Event::CollatorChosen {
					round: 2,
					collator_account: 1,
					total_exposed_amount: Stake { power: 50, amount: 50 },
				},
				Event::CollatorChosen {
					round: 2,
					collator_account: 2,
					total_exposed_amount: Stake { power: 40, amount: 40 },
				},
				Event::CollatorChosen {
					round: 2,
					collator_account: 3,
					total_exposed_amount: Stake { power: 20, amount: 20 },
				},
				Event::CollatorChosen {
					round: 2,
					collator_account: 4,
				...
				Event::CollatorChosen {
					round: 2,
					collator_account: 5,
					total_exposed_amount: Stake { power: 10, amount: 10 },
				},
				Event::NewRound {
					starting_block: 5,
					round: 2,
					selected_collators_number: 5,
					total_balance: Stake { power: 140, amount: 140 },
				},
			);
			assert_eq!(Balances::free_balance(&11), 1);
			...
			assert_events_eq!(
				...
				Event::DelegatorLeftCandidate {
					...
					candidate: 1,
					unstaked_amount: Stake { power: 10, amount: 10 },
					total_candidate_power: Stake { power: 40, amount: 40 },
				},
				Event::DelegatorLeft {
					delegator: 6,
					unstaked_amount: Stake { power: 10, amount: 10 },
			...
			assert_events_eq!(
				...
				Event::NewRound {
					...
					round: 7,
					selected_collators_number: 5,
					total_balance: Stake { power: 130, amount: 130 },
				},
			);
			roll_blocks(3);
			assert_events_eq!(
			...
			assert_events_eq!(Event::Delegation {
				...
				candidate: 1,
				delegator_position: DelegatorAdded::AddedToTop {
					new_total: Stake { power: 50, amount: 50 }
				},
				auto_compound: Percent::zero(),
			});
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				...
				Event::NewRound {
					...
					round: 2,
					selected_collators_number: 1,
					total_balance: Stake { power: 40, amount: 40 },
				},
				Event::JoinedCollatorCandidates {
					account: 4,
					amount_locked: Stake { power: 20, amount: 20 },
					new_total: Stake { power: 60, amount: 60 },
				},
			);
			...
			assert_events_eq!(
				Event::Delegation {
					...
					candidate: 4,
					delegator_position: DelegatorAdded::AddedToTop {
						new_total: Stake { power: 30, amount: 30 }
					},
					auto_compound: Percent::zero(),
				},
				Event::Delegation {
			...
			assert_events_eq!(
				...
				Event::NewRound {
					...
					total_balance: Stake { power: 80, amount: 80 },
				},
			);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_eq!(Event::CandidateLeft {
				ex_candidate: 2,
				unlocked_amount: Stake { power: 400, amount: 400 },
				new_total: Stake { power: 700, amount: 700 },
			},);
		});
}
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				Event::CollatorChosen {
					round: 2,
					collator_account: 1,
					total_exposed_amount: Stake { power: 100, amount: 100 },
				},
				Event::CollatorChosen {
					round: 2,
					collator_account: 2,
					total_exposed_amount: Stake { power: 90, amount: 90 },
				},
				Event::CollatorChosen {
					round: 2,
					collator_account: 3,
					total_exposed_amount: Stake { power: 80, amount: 80 },
				},
				Event::CollatorChosen {
					round: 2,
					collator_account: 4,
					total_exposed_amount: Stake { power: 70, amount: 70 },
				},
				Event::CollatorChosen {
					round: 2,
					collator_account: 5,
					total_exposed_amount: Stake { power: 60, amount: 60 },
				},
				Event::NewRound {
					starting_block: 5,
					round: 2,
					selected_collators_number: 5,
					total_balance: Stake { power: 400, amount: 400 },
				},
			...
			assert_events_eq!(
				Event::CandidateLeft {
					ex_candidate: 6,
					unlocked_amount: Stake { power: 50, amount: 50 },
					new_total: Stake { power: 400, amount: 400 },
				},
				Event::JoinedCollatorCandidates {
					account: 6,
					amount_locked: Stake { power: 69, amount: 69 },
					new_total: Stake { power: 469, amount: 469 },
				},
			);
			roll_to_round_begin(6);
			// should choose top TotalSelectedCandidates (5), in order
			assert_events_eq!(
				...
				Event::CollatorChosen {
					round: 6,
					collator_account: 6,
					total_exposed_amount: Stake { power: 69, amount: 69 },
				},
				Event::NewRound {
					starting_block: 25,
					round: 6,
					selected_collators_number: 5,
					total_balance: Stake { power: 409, amount: 409 },
				},
			);
		});
}
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				...
				Event::NewRound {
					...
					total_balance: Stake { power: 340, amount: 340 },
				},
			);
			// ~ set block author as 1 for all blocks this round
			set_author(2, 1, 100);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				Event::Delegation {
					delegator: 7,
					locked_amount: Stake { power: 80, amount: 80 },
					candidate: 2,
					delegator_position: DelegatorAdded::AddedToTop {
						new_total: Stake { power: 130, amount: 130 }
					},
					auto_compound: Percent::zero(),
				},
				Event::Delegation {
			...
			assert_events_eq!(
				...
				Event::CollatorChosen {
					round: 7,
					collator_account: 3,
					total_exposed_amount: Stake { power: 30, amount: 30 },
				},
				Event::CollatorChosen {
					round: 7,
					collator_account: 4,
			...
			assert_events_eq!(Event::CandidateLeft {
				ex_candidate: 2,
				unlocked_amount: Stake { power: 140, amount: 140 },
				new_total: Stake { power: 120, amount: 120 },
			});
			assert_eq!(
				ParachainStaking::delegator_state(7).unwrap().total(),
				Stake { power: 10, amount: 10 }
			...
			assert_eq!(
				ParachainStaking::get_delegator_stakable_free_balance(&6),
				Ok(Stake { power: 70, amount: 70 })
			);
			assert_eq!(
				ParachainStaking::get_delegator_stakable_free_balance(&7),
				Ok(Stake { power: 90, amount: 90 })
			);
		});
}
...
fn only_top_collators_are_counted() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// 15 + 16 + 17 + 18 + 20 = 86 (top 4 + self bond)
			assert_eq!(collator_state.total_stake_counted, Stake { power: 86, amount: 86 });
			// bump bottom to the top
			assert_ok!(ParachainStaking::delegator_bond_more(RuntimeOrigin::signed(3), 1, 8));
			assert_events_emitted!(Event::DelegationIncreased {
				delegator: 3,
				candidate: 1,
				more: Stake { power: 8, amount: 8 },
				in_top: true,
			});
			let collator_state = ParachainStaking::candidate_info(1).unwrap();
			// 16 + 17 + 18 + 19 + 20 = 90 (top 4 + self bond)
			assert_eq!(collator_state.total_stake_counted, Stake { power: 90, amount: 90 });
			// bump bottom to the top
			assert_ok!(ParachainStaking::delegator_bond_more(RuntimeOrigin::signed(4), 1, 8));
			assert_events_emitted!(Event::DelegationIncreased {
				delegator: 4,
			...
			let collator_state = ParachainStaking::candidate_info(1).unwrap();
			// 17 + 18 + 19 + 20 + 20 = 94 (top 4 + self bond)
			assert_eq!(collator_state.total_stake_counted, Stake { power: 94, amount: 94 });
			// bump bottom to the top
			...
			assert_eq!(collator_state.total_stake_counted, Stake { power: 98, amount: 98 });
			// bump bottom to the top
			...
			assert_eq!(collator_state.total_stake_counted, Stake { power: 102, amount: 102 });
		});
}
...
fn delegation_events_convey_correct_position() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(collator1_state.total_stake_counted, Stake { power: 70, amount: 70 });
			...
			assert_events_emitted!(Event::Delegation {
				delegator: 7,
				locked_amount: Stake { power: 15, amount: 15 },
				candidate: 1,
				delegator_position: DelegatorAdded::AddedToTop {
					new_total: Stake { power: 74, amount: 74 }
				},
				auto_compound: Percent::zero(),
			});
			let collator1_state = ParachainStaking::candidate_info(1).unwrap();
			// 12 + 13 + 14 + 15 + 20 = 70 (top 4 + self bond)
			assert_eq!(collator1_state.total_stake_counted, Stake { power: 74, amount: 74 });
			...
			assert_events_emitted!(Event::DelegationDecreaseScheduled {
				delegator: 6,
				candidate: 1,
				amount_to_decrease: Stake { power: 2, amount: 2 },
				execute_round: 3,
			});
			roll_to(30);
			assert_ok!(ParachainStaking::execute_delegation_request(
			...
			assert_eq!(collator1_state.total_stake_counted, Stake { power: 73, amount: 73 });
			...
			assert_events_emitted!(Event::DelegationDecreaseScheduled {
				delegator: 6,
				candidate: 1,
				amount_to_decrease: Stake { power: 1, amount: 1 },
				execute_round: 9,
			});
			roll_to(40);
			assert_ok!(ParachainStaking::execute_delegation_request(
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				...
				Event::NewRound {
					...
					total_balance: Stake { power: 60, amount: 60 },
				},
			);
			...
			let roll_through_steady_state_round = |round: BlockNumber| -> BlockNumber {
				...
				assert_events_eq!(
					Event::CollatorChosen {
						round: round as u64,
						collator_account: 1,
						total_exposed_amount: Stake { power: 400, amount: 400 },
					},
					Event::CollatorChosen {
						round: round as u64,
						collator_account: 2,
					...
					Event::NewRound {
						...
						round: round as u64,
						selected_collators_number: 4,
						total_balance: Stake { power: 1600, amount: 1600 },
					},
				);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_emitted!(Event::DelegationKicked {
				delegator: 2,
				candidate: 1,
				unstaked_amount: Stake { power: 19, amount: 19 },
			});
			// ensure request DNE
			assert!(!ParachainStaking::delegation_scheduled_requests(&1)
				.iter()
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_events_eq!(Event::DelegationDecreaseScheduled {
				...
				delegator: 2,
				candidate: 1,
				amount_to_decrease: Stake { power: 10, amount: 10 },
			});
			let collator = ParachainStaking::candidate_info(1).expect("candidate must exist");
			assert_eq!(
		...
		.execute_with(|| {
			<DelegationScheduledRequests<Test>>::insert(
				...
				vec![ScheduledRequest {
					delegator: 2,
					when_executable: 3,
					action: DelegationAction::Revoke(Stake { power: 5, amount: 5 }),
				}],
			);
			assert!(ParachainStaking::delegation_request_exists(&1, &2));
		});
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				Event::CollatorChosen {
					round: 4,
					collator_account: 1,
					total_exposed_amount: Stake { power: 500, amount: 500 },
				},
				Event::NewRound {
					starting_block: 15,
					round: 4,
					selected_collators_number: 1,
					total_balance: Stake { power: 500, amount: 500 },
				},
			);
		...
		.execute_with(|| {
			...
			assert_events_eq!(
				Event::CollatorChosen {
					round: 4,
					collator_account: 1,
					total_exposed_amount: Stake { power: 900, amount: 900 },
				},
				Event::NewRound {
					starting_block: 15,
					round: 4,
					selected_collators_number: 1,
					total_balance: Stake { power: 900, amount: 900 },
				},
			);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ParachainStaking::set_inflation(
				RuntimeOrigin::root(),
				Range { min: Perbill::from_percent(2), ideal: Perbill::from_percent(5) }
			));
