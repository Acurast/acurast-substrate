...
use frame_support::{
	pallet_prelude::*,
	traits::{tokens, tokens::WithdrawReasons, LockableCurrency},
};
...
use crate::{
	auto_compound::AutoCompoundDelegations, hooks::StakingHooks, set::OrderedSet, BalanceOf,
	BottomDelegations, CandidateInfo, Config, DelegatorState, Error, Event, Pallet, Round,
	RoundIndex, StakeOf, TopDelegations, Total, COLLATOR_LOCK_ID, DELEGATOR_LOCK_ID,
};
...
impl<Balance: Zero + Add<Balance, Output = Balance>> Zero for Stake<Balance> {
	fn zero() -> Self {
		Stake { amount: Zero::zero(), power: Zero::zero() }
	}
...
impl<Balance: Bounded> Bounded for Stake<Balance> {
	fn min_value() -> Self {
		Stake { amount: Bounded::min_value(), power: Bounded::min_value() }
	}

	fn max_value() -> Self {
		Stake { amount: Bounded::max_value(), power: Bounded::max_value() }
	}
}
...
impl<A, B: Default> Bond<A, B> {
	pub fn from_owner(owner: A) -> Self {
		Bond { owner, stake: Default::default() }
	}
}
...
impl<A: PartialEq, B: PartialEq> PartialEq for CollatorSnapshot<A, B> {
	fn eq(&self, other: &Self) -> bool {
		...
		for (
			BondWithAutoCompound { owner: o1, stake: a1, auto_compound: c1 },
			BondWithAutoCompound { owner: o2, stake: a2, auto_compound: c2 },
...
impl<AccountId, Balance: Default> Default for Delegations<AccountId, Balance> {
	fn default() -> Delegations<AccountId, Balance> {
		Delegations { delegations: Vec::new(), total: Default::default() }
	}
}
...
impl<AccountId, Balance: tokens::Balance> Delegations<AccountId, Balance> {
	...
	pub fn insert_sorted_greatest_to_least(&mut self, delegation: Bond<AccountId, Balance>) {
		...
		match self.delegations.binary_search_by(|x| delegation.stake.cmp(&x.stake)) {
	...
	pub fn lowest_delegation_amount(&self) -> Stake<Balance> {
		self.delegations.last().map(|x| x.stake).unwrap_or(Zero::zero())
	}
	/// Return highest delegation amount
	pub fn highest_delegation_amount(&self) -> Stake<Balance> {
		self.delegations.first().map(|x| x.stake).unwrap_or(Zero::zero())
	}
}
...
impl<Balance: tokens::Balance> CandidateMetadata<Balance> {
		...
		let delegator_added = match self.top_capacity {
			CapacityStatus::Full => {
				...
				if self.lowest_top_delegation < delegation.stake.into() {
					...
					DelegatorAdded::AddedToTop { new_total: self.total_stake_counted }
				} else {
		...
		StakeOf<T>: IsType<Stake<Balance>>,
	{
		...
		if top_delegations.delegations.len() as u32 == max_top_delegations_per_candidate {
			...
			top_delegations.total =
				top_delegations.total.saturating_sub(new_bottom_delegation.stake);
			if matches!(self.bottom_capacity, CapacityStatus::Full) {
		...
		StakeOf<T>: IsType<Stake<Balance>>,
	{
			...
			T::MaxBottomDelegationsPerCandidate::get()
		{
			...
			bottom_delegations.total =
				bottom_delegations.total.saturating_sub(lowest_bottom_to_be_kicked.stake);
		...
		StakeOf<T>: IsType<Stake<Balance>>,
	{
		...
		if !matches!(self.bottom_capacity, CapacityStatus::Empty) {
			...
			bottom_delegations.total =
				bottom_delegations.total.saturating_sub(highest_bottom_delegation.stake);
			self.reset_bottom_data::<T>(&bottom_delegations);
			<BottomDelegations<T>>::insert(candidate, bottom_delegations);
		...
		top_delegations.delegations = top_delegations
			...
			.map(|d| {
				if d.owner != delegator {
					...
					Bond { owner: d.owner, stake: new_amount }
				}
			})
			.collect();
			...
			bottom_delegations.delegations = bottom_delegations
				...
				.map(|d| {
					if d.owner != delegator {
						...
						in_bottom = true;
						Bond { owner: d.owner, stake: d.stake.saturating_add(more) }
					}
				})
				.collect();
			...
			top_delegations.delegations = top_delegations
				...
				.map(|d| {
					if d.owner != delegator {
						...
						is_in_top = true;
						Bond { owner: d.owner, stake: d.stake.saturating_sub(less) }
					}
				})
				.collect();
...
impl<AccountId: PartialEq, Balance: PartialEq> PartialEq for Delegator<AccountId, Balance> {
	fn eq(&self, other: &Self) -> bool {
		...
		for (Bond { owner: o1, stake: a1 }, Bond { owner: o2, stake: a2 }) in
...
impl<AccountId: Ord + Clone, Balance: tokens::Balance> Delegator<AccountId, Balance> {
	pub fn new(id: AccountId, collator: AccountId, stake: Stake<Balance>) -> Self {
		Delegator {
			id,
			delegations: OrderedSet::from(vec![Bond { owner: collator, stake }]),
			total: stake,
	...
	pub fn get_bond_amount(&self, collator: &AccountId) -> Option<Stake<Balance>> {
		self.delegations.0.iter().find(|b| &b.owner == collator).map(|b| b.stake)
	}
}
