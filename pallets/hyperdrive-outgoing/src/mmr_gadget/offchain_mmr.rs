...
use crate::{
	mmr_gadget::{aux_schema, LOG_TARGET},
	utils::NodesUtils,
	HyperdriveApi, LeafIndex, NodeIndex,
};
	...
	I: HyperdriveInstanceName,
{
	...
	fn canonicalize_branch(&mut self, block_hash: B::Hash) {
		...
		let root_hash = match self.client.runtime_api().leaf_meta(block_hash, I::NAME, leaf_index) {
...
mod tests {
	...
	fn canonicalize_and_prune_works_correctly() {
		run_test_with_mmr_gadget(|client| async move {
			...
			let a2 = client.import_block(&BlockId::Hash(a1.hash()), b"a2", vec![1]).await;
			let a3 = client.import_block(&BlockId::Hash(a2.hash()), b"a3", vec![2]).await;
			let a4 = client.import_block(&BlockId::Hash(a3.hash()), b"a4", vec![3]).await;

			let b1 = client.import_block(&BlockId::Number(0), b"b1", vec![0]).await;
			let b2 = client.import_block(&BlockId::Hash(b1.hash()), b"b2", vec![1]).await;
			let b3 = client.import_block(&BlockId::Hash(b2.hash()), b"b3", vec![2]).await;

			let c1 = client.import_block(&BlockId::Number(0), b"c1", vec![0]).await;

			let d4 = client.import_block(&BlockId::Hash(a3.hash()), b"d4", vec![3]).await;
			let d5 = client.import_block(&BlockId::Hash(d4.hash()), b"d5", vec![4]).await;
