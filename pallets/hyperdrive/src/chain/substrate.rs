...
use acurast_core_ink::types::{
	...
	IncomingAction as IncomingActionOnProxy, IncomingActionPayloadV1,
	OutgoingAction as HyperdriveAction, OutgoingActionPayloadV1 as ActionPayloadV1,
	PlannedExecutionV1, VersionedIncomingActionPayload, VersionedOutgoingActionPayload,
};
	...
	AccountConverter: TryFrom<Vec<u8>> + Into<T::AccountId>,
{
	...
	fn decode(encoded: &[u8]) -> Result<ParsedAction<T>, Self::Error> {
		let action =
			HyperdriveAction::decode(encoded).map_err(|_err| Self::Error::CouldNotDecodeAction)?;
