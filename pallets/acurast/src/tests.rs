...
fn test_job_registration() {
	ExtBuilder.build().execute_with(|| {
		...
		assert_eq!(
			events(),
			[
				RuntimeEvent::Acurast(crate::Event::JobRegistrationStoredV2((
					MultiOrigin::Acurast(alice_account_id()),
					initial_job_id + 1
				))),
...
fn test_update_allowed_sources() {
	ExtBuilder.build().execute_with(|| {
		...
		let registration_2 =
			job_registration(Some(bounded_vec![alice_account_id(), bob_account_id()]), false);
		let updates_1 = vec![
			AllowedSourcesUpdate { operation: ListUpdateOperation::Add, item: alice_account_id() },
			AllowedSourcesUpdate { operation: ListUpdateOperation::Add, item: bob_account_id() },
		];
		let updates_2 = vec![
		...
		assert_eq!(
			...
			[
				...
				RuntimeEvent::Acurast(crate::Event::AllowedSourcesUpdatedV2((
					MultiOrigin::Acurast(alice_account_id()),
					initial_job_id + 1
				),)),
				RuntimeEvent::Acurast(crate::Event::AllowedSourcesUpdatedV2((
...
fn test_submit_attestation() {
	ExtBuilder.build().execute_with(|| {
		...
		assert_eq!(Some(attestation.clone()), Acurast::stored_attestation(processor_account_id()));

		assert_eq!(
			events(),
			[RuntimeEvent::Acurast(crate::Event::AttestationStoredV2(processor_account_id()))]
		);
	});
}
...
fn test_set_environment() {
	...
	ExtBuilder.build().execute_with(|| {
		...
		assert_eq!(
			events(),
			[
				RuntimeEvent::Acurast(crate::Event::JobRegistrationStoredV2(job_id.clone())),
				RuntimeEvent::Acurast(
					crate::Event::ExecutionEnvironmentsUpdatedV2(job_id.clone(),)
				),
			]
		);
	});
