...
use crate::{
	...
	types::{MMRError, NodeIndex, Proof, TargetChainNodeHasher},
	utils::NodesUtils,
	Config, HasherError, Leaf, TargetChainConfigOf,
};
use mmr_lib::{self, helper, Merge};
	...
	M: Merge<Item = Node<HashOf<T, I>>>,
{
	...
	pub fn verify_leaves_proof(
		...
		let leaves_positions_and_data = proof
			...
			.collect();
		let root = self.mmr.get_root().map_err(|e| MMRError::GetRoot.log_error(e))?;
	...
	M: Merge<Item = Node<HashOf<T, I>>>,
{
	...
	pub fn push(&mut self, leaf: Leaf) -> Option<NodeIndex> {
		let position =
			self.mmr.push(Node::Data(leaf)).map_err(|e| MMRError::Push.log_error(e)).ok()?;
	...
	pub fn finalize(mut self) -> Result<(NodeIndex, HashOf<T, I>), MMRError> {
		...
		self.mmr.commit(&root_hash).map_err(|e| MMRError::Commit.log_error(e))?;
	...
	M: Merge<Item = Node<HashOf<T, I>>>,
{
	...
	pub fn generate_proof(
		...
		let leaves = positions
			.iter()
			.map(|pos| match mmr_lib::MMRStoreReadOps::get_elem(&store, *pos) {
