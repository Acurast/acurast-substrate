...
//! conjunction with the [`signature_vendored::Signer`] and [`signature_vendored::Verifier`]
//! traits.
//!
//! For example, the [`ring-compat`] crate implements the [`signature_vendored::Signer`]
//! and [`signature_vendored::Verifier`] traits in conjunction with the
...
pub use signature_vendored::{self, Error, Result};
...
/// [`signature_vendored::from_der`] and [`signature_vendored::to_der`] methods.
///
/// # `serde` support
///
	...
	SignatureSize<C>: ArrayLength<u8>,
{
	...
	pub fn split_bytes(&self) -> (FieldBytes<C>, FieldBytes<C>) {
		...
		(GenericArray::clone_from_slice(r_bytes), GenericArray::clone_from_slice(s_bytes))
	}
	...
	SignatureSize<C>: ArrayLength<u8>,
{
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(f, "ecdsa::Signature<{:?}>({:?})", C::default(), self.as_ref())
	}
}
	...
	SignatureSize<C>: ArrayLength<u8>,
{
	...
	fn try_from(bytes: &[u8]) -> Result<Self> {
		...
		Ok(Self { bytes: GenericArray::clone_from_slice(bytes) })
	}
}
