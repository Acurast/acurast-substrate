			...
			let res = rpc.register_method(I::GENERATE_PROOF, |params, context| {
				let (next_message_number, maximum_messages, latest_known_snapshot_number) =
					if params.is_object() {
						...
						(next_message_number, maximum_messages, latest_known_snapshot_number)
					};
	...
	MmrHash: MaybeSerializeDeserialize + Codec + Send + Sync + 'static,
{
	fn snapshot_roots(
		...
		let roots = api
			.snapshot_roots(self.client.info().best_hash, I::NAME, next_expected_snapshot_number)
	...
	fn snapshot_root(
		...
		let api = self.client.runtime_api();
		let root = api
			.snapshot_root(self.client.info().best_hash, I::NAME, next_expected_snapshot_number)
			.map_err(runtime_error_into_rpc_error)?
			.map_err(mmr_error_into_rpc_error)?;
		Ok(root)
	}
