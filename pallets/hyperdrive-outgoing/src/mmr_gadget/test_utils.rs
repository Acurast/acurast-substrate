...
use crate::{
	mmr_gadget::MmrGadget, utils::NodesUtils, HyperdriveApi, LeafIndex, MMRError, NodeIndex,
	SnapshotNumber, TargetChainProof,
};
...
impl MockClient {
	...
	pub async fn import_block(
		...
		for leaf_index in leaf_indices.iter().cloned() {
			...
			runtime_api
				.leaf_meta
				.insert(leaf_index, (parent_hash, MmrHash::from_low_u64_be(leaf_index)));
	...
	pub fn assert_not_canonicalized(&self, blocks: &[&MmrBlock]) {
		self.check_offchain_storage(OffchainKeyType::Canon, blocks, |val, _block, _leaf_index| {
			assert!(val.is_none());
		});
