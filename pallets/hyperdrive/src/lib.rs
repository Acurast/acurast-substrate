...
pub mod pallet {
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn update_state_transmitters(
			...
			let (added, updated, removed) =
				actions.iter().fold((vec![], vec![], vec![]), |acc, action| {
			...
			Self::deposit_event(Event::StateTransmittersUpdate { added, updated, removed });
		...
		pub fn submit_state_merkle_root(
			...
			ensure!(snapshot == expected_snapshot, Error::<T, I>::UnexpectedSnapshot);
			...
			let accepted = StateMerkleRootCount::<T, I>::mutate(
				...
				|submissions| {
					...
					let submissions_count =
						submissions.as_ref().map_or(0usize, |transmitters| transmitters.len());
			...
			if accepted {
				...
				Self::deposit_event(Event::StateMerkleRootAccepted { snapshot, state_merkle_root });
			}
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		fn process_action(proof: &T::Proof) -> Result<(), ProcessMessageResult> {
			let action = proof.message().map_err(|_| ProcessMessageResult::ParsingValueFailed)?;
