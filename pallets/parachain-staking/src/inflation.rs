...
pub fn perbill_annual_to_perbill_round(
	...
	Range { min: annual_to_round(annual.min), ideal: annual_to_round(annual.ideal) }
}
...
mod tests {
	...
	fn mock_round_issuance_range(
		...
		Range { min: round.min * circulating, ideal: round.ideal * circulating }
	}
	#[test]
	fn simple_issuance_conversion() {
		...
		let expected_round_issuance_range: Range<u128> = Range { min: 48_909, ideal: 48_909 };
		let schedule = Range { min: Perbill::from_percent(5), ideal: Perbill::from_percent(5) };
		assert_eq!(
	...
	fn range_issuance_conversion() {
		...
		let expected_round_issuance_range: Range<u128> = Range { min: 29_603, ideal: 39298 };
		let schedule = Range { min: Perbill::from_percent(3), ideal: Perbill::from_percent(4) };
		assert_eq!(
			expected_round_issuance_range,
			mock_round_issuance_range(10_000_000, mock_annual_to_round(schedule, 10))
		);
	...
	fn inflation_does_not_panic_at_round_number_limit() {
		let schedule = Range { min: Perbill::from_percent(100), ideal: Perbill::from_percent(100) };
