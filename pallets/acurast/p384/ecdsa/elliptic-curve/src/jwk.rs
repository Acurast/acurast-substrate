...
impl Debug for JwkEcKey {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		let d = if self.d.is_some() { "Some(...)" } else { "None" };
...
impl<'de> Deserialize<'de> for JwkEcKey {
		...
		D: de::Deserializer<'de>,
	{
		...
		impl<'de> de::Visitor<'de> for FieldVisitor {
				...
				E: de::Error,
			{
				match value {
					...
					b"y" => Ok(Field::Y),
					b"d" => Ok(Field::D),
					_ => Err(de::Error::unknown_field(&String::from_utf8_lossy(value), FIELDS)),
				}
			}
		}
		...
		de::Deserializer::deserialize_struct(
			...
			JWK_TYPE_NAME,
			FIELDS,
			Visitor { marker: PhantomData::<JwkEcKey>, lifetime: PhantomData },
		)
	}
}
...
mod tests {
	...
	fn parse_private_key() {
		...
		assert_eq!(jwk.d.as_ref().unwrap(), "0_NxaRPUMQoAJt50Gz8YiTr8gRTwyEaCumd-MToTmIo");
	}
