...
fn test_single_vest_no_rewards() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AcurastVesting::vest(
			...
			Vesting { stake: 10u128 * UNIT, locking_period: 100u64 }
		));

		System::set_block_number(26);
		assert_ok!(AcurastVesting::cooldown(RuntimeOrigin::signed(alice_account_id()).into(),));
		...
		assert_ok!(AcurastVesting::divest(RuntimeOrigin::signed(alice_account_id()).into(),));
...
fn test_single_vest_rewards() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			AcurastVesting::pool(),
			PoolState { total_power: 10_000_000, total_stake: 10u128 * UNIT, s: (0, 0) },
		);
...
fn test_single_revest_before_cooldown() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AcurastVesting::revest(
			RuntimeOrigin::signed(alice_account_id()).into(),
			// revest the same stake and for same locking_period
			Vesting { stake: 20u128 * UNIT, locking_period: 100u64 }
		));
...
fn test_multiple_vest_rewards() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AcurastVesting::vest(
			...
			Vesting { stake: 20u128 * UNIT, locking_period: 50u64 }
		));

		assert_eq!(
			AcurastVesting::pool(),
			PoolState { total_power: 20_000_000, total_stake: 30u128 * UNIT, s: (0, 0) },
		);
		...
		System::set_block_number(26);
		assert_ok!(AcurastVesting::cooldown(RuntimeOrigin::signed(alice_account_id()).into(),));
		assert_ok!(AcurastVesting::cooldown(RuntimeOrigin::signed(charlie_account_id()).into(),));

		System::set_block_number(76);
		assert_ok!(AcurastVesting::divest(RuntimeOrigin::signed(charlie_account_id()).into(),));
		System::set_block_number(126);
		assert_ok!(AcurastVesting::divest(RuntimeOrigin::signed(alice_account_id()).into(),));
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_err!(
			AcurastVesting::revest(
				RuntimeOrigin::signed(alice_account_id()).into(),
				// revest the same stake and for same locking_period
				Vesting { stake: 9 * UNIT, locking_period: 100u64 }
			),
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_err!(
			AcurastVesting::revest(
				RuntimeOrigin::signed(alice_account_id()).into(),
				// revest the same stake and for same locking_period
				Vesting { stake: 15 * UNIT, locking_period: 99u64 }
			),
...
fn test_maximum_locking_period_exceeded() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_err!(
			AcurastVesting::vest(
				RuntimeOrigin::signed(alice_account_id()).into(),
				Vesting { stake: 10 * UNIT, locking_period: 101 }
			),
