...
use crate::{
	...
	DelegatorOf, DelegatorStatus, Stake, StakeOf,
};
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn delegation_schedule_bond_decrease(
		...
		ensure!(new_amount.amount >= T::MinDelegation::get(), <Error<T>>::DelegationBelowMin);
		...
		ensure!(decrease_amount.amount <= max_subtracted_amount, <Error<T>>::DelegatorBondBelowMin);
	...
	pub(crate) fn revoke_delegator_stake(
		...
		if leaving {
			...
			Self::deposit_event(Event::DelegatorLeft { delegator, unstaked_amount: stake });
		} else {
...
mod tests {
	...
	fn test_cancel_request_with_state_removes_request_for_correct_delegator_and_updates_state() {
		let mut state = Delegator {
			...
			delegations: OrderedSet::from(vec![Bond {
				stake: Stake { power: 100, amount: 100 },
				owner: 2,
			}]),
			total: Stake { power: 100, amount: 100 },
			less_total: Stake { power: 100, amount: 100 },
		...
		let mut scheduled_requests = vec![
			ScheduledRequest {
				delegator: 1,
				when_executable: 1,
				action: DelegationAction::Revoke(Stake { power: 100, amount: 100 }),
			},
			ScheduledRequest {
				delegator: 2,
				when_executable: 1,
				action: DelegationAction::Decrease(Stake { power: 50, amount: 50 }),
			},
		];
		let removed_request =
		...
		assert_eq!(
			...
			Delegator {
				...
				total: Stake { power: 100, amount: 100 },
				less_total: Stake { power: 0, amount: 0 },
				status: crate::DelegatorStatus::Active,
			}
		);
	}
	...
	fn test_cancel_request_with_state_does_nothing_when_request_does_not_exist() {
		let mut state = Delegator {
			id: 1,
			delegations: OrderedSet::from(vec![Bond {
				stake: Stake { power: 100, amount: 200 },
				owner: 2,
			}]),
			total: Stake { power: 100, amount: 200 },
			less_total: Stake { power: 100, amount: 200 },
			status: crate::DelegatorStatus::Active,
		};
		let mut scheduled_requests = vec![ScheduledRequest {
			delegator: 2,
			when_executable: 1,
			action: DelegationAction::Decrease(Stake { power: 50, amount: 100 }),
		}];
		let removed_request =
			<Pallet<Test>>::cancel_request_with_state(&1, &mut state, &mut scheduled_requests);
