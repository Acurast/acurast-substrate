...
macro_rules! construct_async_run {
	{< $runtime:ty > ( |$components:ident, $runner:ident, $cmd:ident, $config:ident| $( $code:tt )* )} => {{
		...
		Some(Subcommand::CheckBlock(cmd)) => {
			...
			match chain_spec.variant() {
				...
				NetworkVariant::Testnet => {
					construct_async_run! {
						<acurast_rococo_runtime::apis::RuntimeApi>(|components, runner, cmd, config| {
				...
				NetworkVariant::Canary => {
					construct_async_run! {
						<acurast_kusama_runtime::apis::RuntimeApi>(|components, runner, cmd, config| {
							Ok(cmd.run(components.client, components.import_queue))
						})
					}
				},
				...
				NetworkVariant::Mainnet => {
					construct_async_run! {
						<acurast_mainnet_runtime::apis::RuntimeApi>(|components, runner, cmd, config| {
							Ok(cmd.run(components.client, components.import_queue))
						})
					}
				},
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				match &config.chain_spec.variant() {
					...
					NetworkVariant::Testnet => {
						service::start_parachain_node::<acurast_rococo_runtime::apis::RuntimeApi>(
							config,
							polkadot_config,
							collator_options,
							id,
					...
					#[cfg(feature = "acurast-kusama")]
					NetworkVariant::Canary => {
						service::start_parachain_node::<acurast_kusama_runtime::apis::RuntimeApi>(
							config,
							polkadot_config,
							collator_options,
							id,
					...
					#[cfg(feature = "acurast-mainnet")]
					NetworkVariant::Mainnet => {
						service::start_parachain_node::<acurast_mainnet_runtime::apis::RuntimeApi>(
							config,
							polkadot_config,
							collator_options,
							id,
