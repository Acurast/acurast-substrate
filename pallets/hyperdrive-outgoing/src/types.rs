...
pub trait TargetChainConfig {
	...
	fn hash_for_target(
		...
		Ok(<Self::Hasher as traits::Hash>::hash(Self::TargetChainEncoder::encode(leaf)?.as_slice()))
	}
}
...
pub enum MMRError {
	...
	#[cfg_attr(feature = "std", error("Error generating proof: no snapshot taken yet"))]
	GenerateProofNoSnapshot,
	/// Error during proof generation when requested snapshot lies in the future.
	#[cfg_attr(feature = "std", error("Error generating proof: snapshot in the future"))]
	GenerateProofFutureSnapshot,
	/// Error during proof generation when requested message start lies in the future.
	#[cfg_attr(feature = "std", error("Error generating proof: message in the future"))]
