...
use frame_support::{
	dispatch::DispatchResultWithPostInfo, ensure, pallet_prelude::RuntimeDebug, traits::Get,
};
	...
	T: Config,
{
	...
	pub(crate) fn delegate_with_auto_compound(
		...
		let mut delegator_state = if let Some(mut state) = <DelegatorState<T>>::get(&delegator) {
			// delegation after first
			ensure!(stake.amount >= T::MinDelegation::get(), Error::<T>::DelegationBelowMin);
			ensure!(
				...
				state.add_delegation(Bond { owner: candidate.clone(), stake }),
				Error::<T>::AlreadyDelegatedCandidate
			);
			state
		} else {
			// first delegation
			ensure!(stake.amount >= T::MinDelegatorStk::get(), Error::<T>::DelegatorBondBelowMin);
		...
		let (delegator_position, less_total_staked) = candidate_state
			.add_delegation::<T>(&candidate, Bond { owner: delegator.clone(), stake })?;
		...
		let net_total_increase =
			if let Some(less) = less_total_staked { stake.saturating_sub(less) } else { stake };
