...
use crate::{
	instances::HyperdriveInstance,
	mmr::Merger,
	traits::MMRInstance,
	types::{Node, TargetChainProofLeaf},
};
...
pub mod pallet {
	...
	use crate::{default_weights::WeightInfo, traits::MMRInstance};
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	pub fn generate_proof(
		...
		ensure!(next_message_number <= last_message_excl, MMRError::GenerateProofFutureMessage);
	...
	pub fn generate_target_chain_proof(
		...
		proof
			.map(|(leaves, proof)| {
				...
				Ok(TargetChainProof { leaves, mmr_size, items: proof.items })
			})
			.transpose()
	}
	...
	pub fn snapshot_roots(
		...
		(next_expected_snapshot_number..next_snapshot_number).into_iter().map(
			move |snapshot_number| {
