// DISABLED together with the versioned transaction extensions (see `types.rs`).
//
// `VerifySignature` is what lets a v5 general transaction establish its origin, so it is only
// needed when a transaction extension version other than 0 exists. Kept here so re-enabling is
// a matter of uncommenting this file, its `mod` line in `pallets.rs`, the pallet index in
// `lib.rs`, the benchmark entry, and the Cargo dependency.
//
// use crate::Runtime;
