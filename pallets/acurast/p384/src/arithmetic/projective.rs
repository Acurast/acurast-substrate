...
impl ProjectivePoint {
	...
	pub const GENERATOR: Self =
		Self { x: AffinePoint::GENERATOR.x, y: AffinePoint::GENERATOR.y, z: FieldElement::ONE };
	...
	pub fn to_affine(&self) -> AffinePoint {
		self.z
			.invert()
			.map(|zinv| AffinePoint { x: self.x * &zinv, y: self.y * &zinv, infinity: 0 })
	...
	fn add(&self, other: &ProjectivePoint) -> ProjectivePoint {
		weierstrass::add((self.x, self.y, self.z), (other.x, other.y, other.z), CURVE_EQUATION_B)
			.into()
	}
	...
	fn mul(&self, k: &Scalar) -> ProjectivePoint {
		...
		pc[1] = *self;
		for i in 2..16 {
			pc[i] = if i % 2 == 0 { pc[i / 2].double() } else { pc[i - 1].add(self) };
		}
...
impl From<AffinePoint> for ProjectivePoint {
	fn from(p: AffinePoint) -> Self {
		let projective = ProjectivePoint { x: p.x, y: p.y, z: FieldElement::ONE };
