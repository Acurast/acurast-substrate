...
pub mod pallet {
		...
		frame_system::Config + pallet_acurast::Config + pallet_acurast_hyperdrive_ibc::Config<I>
	{
		...
		type AssetId: Parameter + Member + MaxEncodedLen + Copy + Into<u32> + From<u32>;
	...
	#[pallet::type_value]
	pub fn InitialEthereumContract<T: Config<I>, I: 'static>() -> AccountId20 {
		T::EthereumContract::get()
	}

	#[pallet::type_value]
	pub fn InitialSolanaContract<T: Config<I>, I: 'static>() -> AccountId32 {
		T::SolanaContract::get()
	}

	#[pallet::storage]
	#[pallet::getter(fn ethereum_contract)]
	pub type EthereumContract<T: Config<I>, I: 'static = ()> =
		StorageValue<_, AccountId20, ValueQuery, InitialEthereumContract<T, I>>;

	#[pallet::storage]
	#[pallet::getter(fn solana_contract)]
	pub type SolanaContract<T: Config<I>, I: 'static = ()> =
		StorageValue<_, AccountId32, ValueQuery, InitialSolanaContract<T, I>>;
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn update_ethereum_contract(
			...
			ensure_root(origin)?;
			<EthereumContract<T, I>>::set(contract.clone());
		...
		pub fn update_solana_contract(
			...
			ensure_root(origin)?;
			<SolanaContract<T, I>>::set(contract.clone());
		...
		pub fn do_transfer_native(
			...
			if transfer_nonce.is_none() {
				...
				if !amount.is_zero() {
					...
					.map_err(|_| Error::<T, I>::TransferToVaultFailed)?;
				}
			}
			...
			if !fee.is_zero() {
                ...
                &source,
                    ...
                    Preservation::Preserve,
                ).map_err(|_| Error::<T, I>::TransferToFeeVaultFailed)?;
			}
		...
		fn process(message: MessageBody<T::AccountId, T::AccountId>) -> DispatchResultWithPostInfo {
			let (proxy, action) = match message.sender {
				SubjectFor::<T>::Ethereum(Layer::Contract(contract_call)) => {
					if contract_call.contract != Self::ethereum_contract() {
				...
				SubjectFor::<T>::Solana(Layer::Contract(contract_call)) => {
					if contract_call.contract != Self::solana_contract() {
						Err(Error::<T, I>::InvalidSender)?
					}
