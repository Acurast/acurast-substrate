	...
	AccountConverter: TryFrom<Vec<u8>> + Into<T::AccountId>,
{
	...
	fn calculate_root(self: &Self) -> Result<[u8; 32], Self::Error> {
		let account_proof: Vec<Vec<u8>> =
			self.account_proof.iter().map(|node| node.to_vec()).collect();

		let storage_proof: Vec<Vec<u8>> =
			self.storage_proof.iter().map(|node| node.to_vec()).collect();
		...
		let account_path = Keccak256::hash(storage_owner_address.as_ref()).as_bytes().to_vec();
	...
	fn message(self: &Self) -> Result<ParsedAction<T>, Self::Error> {
		...
		let value = if self.value.len() > 32 { &self.value[32..] } else { &self.value };
		...
		match action {
			...
			RawAction::SetJobEnvironment => {
				...
				let job_id =
					(MultiOrigin::Ethereum(origin_address.clone()), set_job_environments.jobId);
				...
				Ok(ParsedAction::SetJobEnvironment(job_id, BoundedVec::truncate_from(variables)))
			},
