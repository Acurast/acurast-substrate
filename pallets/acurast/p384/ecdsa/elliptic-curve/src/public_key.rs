	...
	C: Curve + ProjectiveArithmetic,
{
	...
	pub fn from_secret_scalar(scalar: &NonZeroScalar<C>) -> Self {
		...
		Self { point: (C::ProjectivePoint::generator() * scalar.as_ref()).to_affine() }
	}
	...
	FieldSize<C>: ModulusSize,
{
	fn cmp(&self, other: &Self) -> Ordering {
		...
		self.to_encoded_point(false).cmp(&other.to_encoded_point(false))
	}
}
	...
	FieldSize<C>: ModulusSize,
{
	fn to_public_key_der(&self) -> pkcs8::spki::Result<der::Document> {
		...
		pkcs8::SubjectPublicKeyInfo { algorithm, subject_public_key: public_key_bytes.as_ref() }
			.try_into()
	}
}
	...
	FieldSize<C>: ModulusSize,
{
	fn to_string(&self) -> String {
		self.to_public_key_pem(Default::default()).expect("PEM encoding error")
	}
}
...
mod tests {
	...
	fn from_encoded_point_rejects_identity() {
		...
		assert!(bool::from(PublicKey::from_encoded_point(&identity).is_none()));
	}
}
