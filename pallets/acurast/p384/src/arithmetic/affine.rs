...
impl<'de> Deserialize<'de> for AffinePoint {
	...
	{
		EncodedPoint::deserialize(deserializer)?.try_into().map_err(de::Error::custom)
	}
}
...
mod tests {
	...
	fn uncompressed_to_compressed() {
		...
		let res = AffinePoint::from_encoded_point(&encoded).unwrap().to_encoded_point(true);
	...
	fn compressed_to_uncompressed() {
		...
		let res = AffinePoint::from_encoded_point(&encoded).unwrap().to_encoded_point(false);
