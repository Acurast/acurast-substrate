...
use crate::{mmr::NodeOf, mock::*, stub::*, *};
...
fn should_start_empty() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			Some(hex("cb0c62b3cab675b8b9ca7106c5bbe219048c1af50b1c9ed360443e7df108189d"))
		);
		assert_eq!(
			crate::RootHash::<Test>::get(),
...
fn should_append_to_mmr_when_send_message_is_called() {
	...
	let (parent_b1, parent_b2) = ext.execute_with(|| {
		...
		assert_eq!(
			...
			(
				None,
				None,
				Some(hex("bf9f9624420719df2fcbcfd14d0071aa8668c3433f6013bb253f4b4e03587b77")),
				None,
				root_1,
			)
		);
	...
	assert_eq!(
		offchain_db
			.get(&HyperdriveOutgoing::node_temp_offchain_key(0, parent_b1, root_0))
			.map(decode_node),
	...
	assert_eq!(
		offchain_db
			.get(&HyperdriveOutgoing::node_temp_offchain_key(1, parent_b2, root_1))
			.map(decode_node),
	...
	assert_eq!(
		offchain_db
			...
			.map(decode_node),
		Some(Node::Hash(hex("bf9f9624420719df2fcbcfd14d0071aa8668c3433f6013bb253f4b4e03587b77",)))
	);
...
fn should_construct_larger_mmr_correctly() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			(
				Some(hex("d3abc9027d4a827c6acf44806672576b48a26e363a5d329b8e43327230a9f811")),
				Some(hex("b2206bdb47aae933a96dd8860d23eb039d6add38cd85e892150e1c8c16831ba5")),
				Some(hex("2762adf78b467c382b52cdb09cb8f885fea8719b1861369b4202497f645ad402")),
	...
	ext.execute_with(|| {
		...
		assert_eq!(
			...
			Some((
				...
				Proof { leaf_indices: vec![0, 1, 2, 3, 4, 5, 6], leaf_count: 7, items: vec![] }
			))
		);
		assert_eq!(
			...
			Ok(Some((
				...
				Proof { leaf_indices: vec![0, 1, 2], leaf_count: 3, items: vec![] }
			)))
		);
...
fn verification_should_be_stateless() {
	...
	assert_eq!(Pallet::<Test>::verify_proof_stateless(root_a, leaves_a, proof_a), Ok(()));
	assert_eq!(Pallet::<Test>::verify_proof_stateless(root_b, leaves_b, proof_b), Ok(()));
}
...
fn should_generate_maximum_messages() {
	...
	ext.execute_with(|| {
		...
		let (leaves, proof) = Pallet::<Test>::generate_proof(2, Some(3), 0).unwrap().unwrap();
