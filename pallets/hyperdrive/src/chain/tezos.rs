...
use sp_std::{prelude::*, str::FromStr, vec};
use tezos_core::{types::encoded::Address as TezosAddress, Error as TezosCoreError};
use tezos_michelson::{
	micheline::{primitive_application, primitive_application::PrimitiveApplication, Micheline},
	michelson::{
		data::{
			self, try_bytes, try_int, try_nat, try_string, Bytes, Data, Elt, Int, Nat, Pair,
			Sequence,
		},
		types::{address, bool as bool_type, bytes, map, nat, option, pair, set, string},
		ComparableTypePrimitive,
	},
	Error as TezosMichelineError,
};
...
use crate::{
	traits,
	types::{derive_proof, MessageParser, RawAction, StateKey, StateOwner, StateProof, StateValue},
	CurrentTargetChainOwner, MessageIdentifier, ParsedAction,
};
		...
		Ok(match action {
			RawAction::RegisterJob => {
				...
				ParsedAction::RegisterJob(
					(MultiOrigin::Tezos(bounded_address(&origin)?), job_id_sequence),
					registration,
				)
			},
			...
			RawAction::SetJobEnvironment => {
				...
				let job_id = (MultiOrigin::Tezos(bounded_address(&origin)?), job_id_sequence);
...
fn parse_message(encoded: &[u8]) -> Result<(RawAction, TezosAddress, Bytes), TezosValidationError> {
	...
	let origin: TezosAddress =
		try_address(iter.next().ok_or(TezosValidationError::MissingField(FieldError::ORIGIN))?)?;
	let body: Bytes =
		try_bytes(iter.next().ok_or(TezosValidationError::MissingField(FieldError::PAYLOAD))?)?;
...
>(
	...
	(JobIdSequence, JobRegistration<AccountId, MaxAllowedSources, Extra>),
	...
	MaxSlots: ParameterBound,
{
	...
	let allow_only_verified_sources: bool = try_bool(
		iter.next()
			.ok_or(TezosValidationError::MissingField(FieldError::AllowOnlyVerifiedSources))?,
	)?;
	...
	let instant_match = try_option(
		...
		|value| {
			let sources = try_sequence(value, |planned_execution| {
				...
				Ok(PlannedExecution { source, start_delay })
			})?;
	...
	let reward = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::Reward))?)?;
		let v: u128 = v.to_integer()?;
		v.into()
	};
	let slots = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::Slots))?)?;
		v.to_integer()?
	};
	let job_id = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::JobId))?)?;
		v.to_integer()?
	};
	let memory = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::Memory))?)?;
		v.to_integer()?
	};
	...
	let duration = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::Duration))?)?;
		v.to_integer()?
	};
	let end_time = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::EndTime))?)?;
		v.to_integer()?
	};
	let interval = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::Interval))?)?;
		v.to_integer()?
	};
	let max_start_delay = {
		let v: Int = try_int(
	...
	let start_time = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::StartTime))?)?;
		v.to_integer()?
	};

	let script = {
		let script: Vec<u8> = (&try_bytes::<_, Bytes, _>(
			iter.next().ok_or(TezosValidationError::MissingField(FieldError::Script))?,
		)?)
			.into();
		script.try_into().map_err(|_| TezosValidationError::ScriptOutOfBounds)?
	};
	let storage = {
		let v: Int =
			try_int(iter.next().ok_or(TezosValidationError::MissingField(FieldError::Storage))?)?;
		v.to_integer()?
	};
	...
	Ok((
		...
		JobRegistration {
			...
			allowed_sources,
			allow_only_verified_sources,
			schedule: Schedule { duration, start_time, end_time, interval, max_start_delay },
			memory,
			network_requests,
			storage,
			required_modules,
...
fn parse_set_job_environment_payload<T: pallet_acurast::Config, ParsableAccountId>(
	...
	(JobIdSequence, BoundedVec<(T::AccountId, EnvironmentFor<T>), T::MaxSlots>),
	TezosValidationError,
>
where
	ParsableAccountId: TryFrom<Vec<u8>> + Into<T::AccountId>,
{
	...
	let processors: Vec<(
		T::AccountId,
		Vec<(BoundedVec<u8, T::EnvKeyMaxSize>, BoundedVec<u8, T::EnvValueMaxSize>)>,
	)> = try_sequence::<
		(T::AccountId, Vec<(BoundedVec<u8, T::EnvKeyMaxSize>, BoundedVec<u8, T::EnvValueMaxSize>)>),
		_,
	>(iter.next().ok_or(TezosValidationError::MissingField(FieldError::Processors))?, |entry| {
		...
		let source = {
			...
			let parsed: ParsableAccountId =
				source.try_into().map_err(|_| TezosValidationError::AddressParsing)?;
	...
	let public_key_bytes: Bytes = try_bytes::<_, Bytes, _>(
		iter.next().ok_or(TezosValidationError::MissingField(FieldError::PublicKey))?,
	)?;
...
mod tests {
	...
	use crate::{instances::TezosInstance, mock::*, Config};
